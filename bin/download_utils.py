...
def search_entrez_ids(ids: list) -> dict:
    """search_entrez_ids.
    ...
    Returns:
        dict: mapping new id to old id
    """

    output = {}
    for id_ in ids:
        handle = Entrez.esearch(db="Protein", retmax=10, term=id_, idtype="acc")
        entries = Entrez.read(handle)
        if len(entries["IdList"]) == 1:
            new_id = entries["IdList"][0]
            logging.debug(f"Replacing id {id_} with id {new_id}")
            output[new_id] = id_
        elif len(entries["IdList"]) != 1:
            logging.warning(
                f"Found {len(entries['IdList'])} alt ids for old id {id_}")
        handle.close()
    return output


def extract_entrez_prot(ids: list, email_address: str = "") -> Tuple[dict, set]:
    """extract_entrez_prot.

    Helper fn to grab protein ids from entrez. 

    Args: 
        ids (List[str]): list of entrez protein ids
        email_address (str): Use to tell the datbase who's contacting
    Return:
        dict: Mapping all recovered IDs to coding sequences
        set: Set of all proteins found in the search at all 
    """
    results_dict = {}
    Entrez.email = email_address

    # Batch the searches
    num_batches = np.ceil(len(ids) / MAX_BATCH_SIZE)
    found_in_search = set()

    for id_set in np.array_split(list(ids), num_batches):
        try:
        ...
        # Catch urrllib httperror
        except OSError:
            output = []
            logging.warning("Error in efetch")

        for entry in output:
            feat_tbl = entry["GBSeq_feature-table"]
            locus_name = entry["GBSeq_locus"]
            if locus_name not in ids:
                locus_name = entry["GBSeq_primary-accession"]

            if locus_name not in ids:
                logging.warning(f"Locus name {locus_name} not in search ids")

            found_in_search.add(locus_name)
            num_cds = 0
            num_seqs = 0
            seq = None
            for j in feat_tbl:
                if j['GBFeature_key'] == "CDS":
                    num_cds += 1
                    if j["GBFeature_quals"]:
                        for qualifier in j["GBFeature_quals"]:
                            if ("GBQualifier_name" in qualifier and
                                    qualifier["GBQualifier_name"] ==
                                    "translation" and
                                    "GBQualifier_value" in qualifier):

                                seq = qualifier["GBQualifier_value"]
                                num_seqs += 1

            if num_cds == 1 and num_seqs == 1 and seq:
                results_dict[locus_name] = seq
            elif (num_cds <= 1 and num_seqs != 1 and
                  "GBSeq_moltype" in entry and
                  entry["GBSeq_moltype"].upper() == "AA" and
                  "GBSeq_sequence" in entry):
                results_dict[locus_name] = entry["GBSeq_sequence"]
                logging.debug(
                    f"Found sequence {locus_name} in primary entry: {results_dict[locus_name]}"
                )
            else:
                logging.warning(
                    f"When searching genbank, found {num_cds} cds and {num_seqs} seqs for id {locus_name}"
                )

        handle.close()

    return results_dict, found_in_search


def debug_entrez():
    """Helper fn for debugging"""
    ids = [
        "AFH12530", "AXE2_TALPU", "AE010289", "KJ159967", "KC505218",
        "HV538330", "AF288905", "AF288898", "AF288917", "AF288906", "AF288904",
        "AF288900", "AF288901", "AF288902", "AF288911", "AF288899"
    ]

    temp, found_in_search = extract_entrez(ids=ids, email_address="")
    not_found = set(ids).difference(found_in_search)
    new_id_mapping = entrez_ids(not_found)
    temp_new, found_new = extract_entrez(ids=list(new_id_mapping.keys()))
...
def download_enzymes(f_name : str, 
    ...
    try: 
            ...
            while total_lines_to_read is None or num_lines_read < total_lines_to_read: 
                ...
                if num_requests == 0 : 
                    ...
                    if total_lines_to_read is None: 
                        ...
                        total_lines_to_read = min(header_lines,
                                                  total_lines_to_read) 
        ...
        # Recursive call
        download_enzymes(f_name, step_size, total_lines_to_read,
                         read_timeout* 2, columns, query)

UNIPROT_COLUMNS = ["id", "organism", "ec", "sequence", "feature(ACTIVE SITE)",
                   "organism-id", "length", "annotation score", "protein names"]

def download_enzyme_list(f_name : str, 
                         enzyme_list : list, 
                         step_size :int = 1000,
                         read_timeout : int = 240, 
                         columns : List[str] = ["id", "sequence"], 
    ...
    splits = np.array_split(enzyme_list, 
                            num_partitions)
    ...
    with open(f_name, write_code) as f:
        for split_index, split in tqdm(enumerate(splits)): 
            ...
            try: 
                ...
                # Recursive call
                download_enzyme_list(f_name,
                                     list(np.concatenate(splits[split_index:])), 
                                     step_size,
                                     read_timeout* 2, 
                                     columns)

def replace_uniparc_ids(f_name : str, 
                        step_size :int = 1000,
                        read_timeout : int = 240, 
                        columns : List[str] = ["id", "sequence"]): 
    """replace_uniparc_ids. 
        ...
        for line, j in enumerate(open(f_name, "r")): 
            ...
            if line == 0: 
                ...
                if len(headers) != len(columns): 
                    raise RuntimeError( """Require the file being opened to
                ...
                if alt_name_index is None or id_index is None or seq_index is None : 
                    raise RuntimeError(f"File {f_name} has no col Protein names or Entry or sequence ")
            # Parse body to get sequences that have been merged
                ...
                if "Merged" in alt_name: 
                    ...
                    if r is None or len(r.groups()) !=  1: 
                        raise RuntimeError(f"Found merged but failed to match single regex to {alt_name}")
                    else: 
    ...
    download_enzyme_list(temp_uniparc_name, deleted_ids,  step_size,
                         read_timeout, uniparc_cols, "uniparc")
    ...
    logging.info(f"""Looking through the uniprot downloads, we found:
    ...
    download_enzyme_list(tf_name, new_values,  step_size, read_timeout,
                         columns)
...
def download_uniprot_uniparc_list(f_name : str, enzyme_list : list, 
                                  step_size :int = 200, 
                                  read_timeout : int = 240):
    ...
    Args: 
        ...
        read_timeout (int) : Seceonds to wait before timing out 
    """
    download_enzyme_list(f_name, [f"id:{i}" for i in enzyme_list],
                         database="uniprot", columns=UNIPROT_COLUMNS, 
                         step_size=step_size, 
                         read_timeout=read_timeout)
    replace_uniparc_ids(f_name, step_size=step_size, read_timeout=read_timeout,
